...
class CustomerService extends BaseService
{
    ...
    public function createCustomer($request)
    {
        ...
        try {
            ...
            $relationshiptype = quote_smart($request->relationshiptype);
            $usercustomertype = quote_smart($request->usercustomertype);
...
//                $maxValue = addPadding( $maxValue, 8, '0');
                ...
                if ($service_product_id == 4){
                    ...
                    $approved_product = '1'; //Admin kiem tra neu da thanh toan goi dich vu thi enable len trong table users: neu chon goi mien phi thi kich hoat = 1 (hoat dong)
                }
    ...
    public function getListCustomerFromCustomerIdParent($customer_id, $usercustomertype)
    {
        $listData = DB::table('users')
                        ->leftjoin('customers', 'users.customer_id', '=', 'customers.id')
                        ->select('users.id','customer_id','fullname','phone','users.email','usercustomertype','relationshiptype','dependent','birthday','role_member','users.created_at')
                        ->where('users.customer_id_parent', '=', "$customer_id")
                        ->where('customers.usercustomertype', 'like', "%$usercustomertype%")
    ...
    public function createCustomerFromUser($request)
    {
        DB::beginTransaction();
        try {
            $maxValue = app(Customer::class)::max('customercode');
            ...
            $rolemembertype = quote_smart($request->rolemembertype);
            $type_user = quote_smart($request->type_user);
            $customer_id_parent = quote_smart($request->customer_id_parent);
            ...
            $blocked_code = "0";// 0: Mo tai khoan  1: Khoa tai khoan  => bang tai khoan dang nhap khach hang user
            //Neu chon goi mien phí thi chuyen trang thai sang tai khoan -> da duyet, hop dong -> hoat dong
            //Do user tao tai khoan con nen se dung goi tai khoan cha va kich hoat dich vu
            if ($service_product_id != "4"){//khac goi mien phi
                $approved_code = 1;// $service_product_id = 4 -> chon goi mien phi, cac goi khac deu tao user mien phi
                $approvestatustype_code = "A";//Da duyet
                $contractstatustype_code = "2";//Hoat dong
                $blocked_code = "0";//Mo tai khoan
...
//                $maxValue = addPadding( $maxValue, 8, '0');
                ...
                $approved_product = '0'; //Admin kiem tra neu da thanh toan goi dich vu thi enable len trong table users
                //Neu khach hang chon goi mo tai khoan mien phi = 4
                //Do khach hang tao user con tu user cha nen kich hoat tai khoan de su dung luon
                if ($service_product_id != 4){
                    $begin_at_product = null; //Ngay bat dau su dung goi dich vu = ngay mo hop dong
                    $finish_at_product =  null; //Ngay ket thuc goi dich vu
                    $approved_product = '1'; //Admin kiem tra neu da thanh toan goi dich vu thi enable len trong table users: neu chon goi mien phi thi kich hoat = 1 (hoat dong)
