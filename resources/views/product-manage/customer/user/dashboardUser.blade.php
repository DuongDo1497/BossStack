...
@section('content')
  ...
  <div class="section customer-user__dashboard customer-main">
    ...
    <form role="form" action="{{ route('customers-dashboardUser') }}?continue=true" method="post" name="frm"
      id="frm">
      {{ csrf_field() }}
      ...
      <div class="box-content">
        <div class="box box-primary">
          ...
          @else
            <div class="alert alert-success">Bạn đã tạo đủ số lượng {{ $collections->count() }} User Thành viên trong gói.
            </div><br>
          @endif
                  ...
                  @foreach ($collections as $item)
                    ...
                    <tr>
                      ...
                      <td class="fixed fixed-3">
                        <a
                          href="{{ route('customers-inforUser', ['id' => $item->customer_id]) }}">{{ $item->fullname }}</a>
                      </td>
                      <td class="text-center">
                        {{ $item->usercustomertype == '' ? '' : $usercustomertypes[$item->usercustomertype] }}</td>
                      <td class="text-center">
                        {{ $item->relationshiptype == '' ? '' : $relationshiptypes[$item->relationshiptype] }}</td>
                    ...
                    @endif
                      ...
                      <td class="text-center">{{ $item->created_at == null ? '' : ConvertSQLDate($item->created_at, 1) }}
                      </td>
                    </tr>
                  @endforeach
                </tbody>
